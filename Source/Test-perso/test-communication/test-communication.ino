...
namespace seatalk {
  ...
  boolean writeDatagram(uint16_t data[]) {
    ...
    for (int ii = 0; ; ii += 2) {
      ...
      case Cmd:
        // NB: set command bit
        Serial.write9(data[ii + 1],true);     
        break;

      case Nibble:
        // NB: nibbles must come in pairs, with most significant first
        if (data[ii + 2] != Nibble) return false;
        Serial.write9((data[ii + 1] << 4) | (data[ii + 3] & 0x0f), true);
        ii += 2; // skip over next nibble which we've already consumed
        break;

      case Att:
      case Byte:
        Serial.write9(data[ii + 1], false);     // mask high byte
        break;

      case Int:
      case Int10:
        Serial.write9(data[ii + 1], true);    // LSB first
        Serial.write9(data[ii + 1], false);       // MSB second
        break;
